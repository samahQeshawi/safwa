...
<script type="text/javascript">
   $(function () {
         var newDriver = new Option('{{@$transaction->trip->trip_no}}', '{{@$transaction->trip_id}}', true, true);
        $('#trip_id').append(newDriver).trigger('change');
    
     $('#trip_id').select2({
        placeholder: 'Select Location',
        ajax: {
            url: '{{ route("transaction.search.trip") }}',
            dataType: 'json',
            delay: 250,
