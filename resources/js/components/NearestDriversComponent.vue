...
export default {
    ...
    created() {
        this.fetchNearestDrivers();
        Echo.private("driverLocation").listen(
            "UpdateDriverLocationEvent",
            event => {
                this.drivers = this.drivers.filter( //Remove the updated driver location
                    u => u.user_id != event.user_id
                );
                this.drivers.push(event); //Add the driver new location
            }
        );
    },
    ...
    methods: {
        fetchNearestDrivers() {
            axios.get("/customers/nearest-drivers/"+this.user).then(response => {
