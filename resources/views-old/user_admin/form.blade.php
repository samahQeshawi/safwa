...
<div class="form-group {{ $errors->has('gender') ? 'has-error' : '' }}">
    <label for="gender" class="col-md-2 control-label">{{ trans('user_admin.gender') }}</label>
    <div class="col-md-10">
        <select class="form-control" id="gender" name="gender">
        	    <option value="" style="display: none;" {{ old('gender', optional($user_admin)->gender ?: '') == '' ? 'selected' : '' }} disabled selected>{{ trans('user_admin.gender__placeholder') }}</option>
        	@foreach (['Male' => trans('user_admin.gender_male'),
'Female' => trans('user_admin.gender_female'),
'Others' => trans('user_admin.gender_others')] as $key => $text)
			    <option value="{{ $key }}" {{ old('gender', optional($user_admin)->gender) == $key ? 'selected' : '' }}>
			    	{{ $text }}
			    </option>
			@endforeach
        </select>
