...
class CustomersTripsController extends Controller
{
    ...
    public function userRatingCustomer($id)
    {
        $rating   =   Rating::where('rated_by', $id)->where('user_type','customer')->first();
	...
	$newdata['trip']['rating'] =  isset($row->customer_id) ? $this->userRatingCustomer($row->customer_id)  : '';
    ...
    public function addTripDetails(Request $request)
    {
       ...
       //next_number = str_pad($trip_no, 10, "0", STR_PAD_LEFT);
        ...
        // Set to default payement method of customer
        $payment_option = UserPaymentOption::where('user_id', $user_id)->where('is_default', 1)->with('paymentMethod')->get()->first();
        // return $payment_option;
        $data['payment_method'] = $payment_option->payment_title;
        $data['payment_method_id'] = $payment_option->payment_method_id;
        ...
        $list = $this->push_nearest_drivers($data['from_location_lat'], $data['from_location_lng'], $category->id);
    ...
    public function UpdateTripStatus(Request $request)
    {
        ...
        if ($request->status == '8') { //Apply coupon on trip completion
            ...
            if( isset($request->payment_method_id) ) {
                $payment_option = UserPaymentOption::where('user_id', $trip->customer_id )->where('payment_method_id', $request->payment_method_id )->with('paymentMethod')->get()->first();
            } else {
                $payment_option = UserPaymentOption::where('user_id', $trip->customer_id )->where('is_default', 1)->with('paymentMethod')->get()->first();
            }
            ...
            if($trip->payment_status == 0) {
                $notify = [];
                // if customer choose wallet or Card
                if($payment_option->payment_method_id == config('constants.Wallet') || $payment_option->payment_method_id == config('constants.Card') || $payment_option->payment_method_id == config('constants.Mada')) {
                    $trip->payment_method = $payment_option->payment_title;
                    $trip->payment_method_id = $payment_option->payment_method_id;
                    $trip->collected_money = $trip->final_amount;
                    if($payment_option->payment_method_id == config('constants.Wallet')) {
                        $trip->payment_status = 1;
                        $trip->status = '9';
                    }
                    $trip->save();

                    $amount_to_customer_wallet = 0 - $trip->final_amount;
                    ...
                    $transaction_data['amount'] = $amount_to_driver_wallet;
                    $transaction_data['done_by']  = auth()->user()->id;
                    $transaction_data['sender_id'] = $trip->customer_id;
                    $transaction_data['receiver_id'] = $trip->driver_id;
                    $transaction_data['trip_id'] = $trip->id;
                    $transaction_data['note'] = trans('api.driver.money_adjusted');
                    $transaction_data['status'] = "Completed";
                    Transaction::create($transaction_data);

                    // Format Trip Details
                    $trip = Trip::with('car', 'service', 'paymentMethod','customer')->where('id', $request->trip_id)->first();

                    //Format Trip Details
                    $trip_details = $this->formatTripDetailsWithKey($trip);

                    $data = ["trip_details" => $trip_details];
                    // Send money collected using push. to driver
                    $driver_push = AuthController::sendPushNotification([
                        'title' => "Money Collected for trip #" . $trip->trip_no,
                        'body' => "Money collected to your wallet for trip #" . $trip->trip_no,
                        'user_id' => $trip->driver_id,
                    ], $trip_details);
                    array_push($notify, ["user_id" => $trip->driver_id, "push" => $driver_push]);

                    if($payment_option->payment_method_id == config('constants.Wallet')) {
                        $customer_push = AuthController::sendPushNotification([
                            'title' => "Money Collected for trip #" . $trip->trip_no,
                            'body' => "Money collected from your wallet for trip #" . $trip->trip_no,
                            'user_id' => $trip->customer_id,
                        ], $data);
                        array_push($notify, ["user_id" => $trip->customer_id, "push" => $customer_push]);
                        // return $trip_details;
                    } else if($payment_option->payment_method_id == config('constants.Card') || $payment_option->payment_method_id == config('constants.Mada')) {
                        $customer_push = AuthController::sendPushNotification([
                            'title' => "Trip Completed for trip #" . $trip->trip_no,
                            'body' => "Please pay trip amount for trip #" . $trip->trip_no,
                            'user_id' => $trip->customer_id,
                        ], $data);
                        array_push($notify, ["user_id" => $trip->customer_id, "push" => $customer_push]);
                        // return $trip_details;
                    ...
                    $status_code   = 200;
                    return response(['data' => $data , 'message' => $message, 'status_code' => $status_code, 'notify' => $notify ]);
                }
            }
            else {
                $message = 'Trip was payed before!';
                $status_code    = 422;
                $data = ["trip_details" => $trip_details];
                return response(['data' => $data, 'message' => $message, 'status_code' => $status_code]);
            }
        ...
        $data = ["trip_details" => $trip_details];
        //Call the NewTripHasCreatedEvent with this new trip
        // event(new TripStatusHasUpdatedEvent($trip, $data));
        AuthController::sendPushNotification([
            'title' => "Trip status has been updated for #" . $trip->trip_no,
            'body' => "Trip status has been updated for #" . $trip->trip_no,
            'user_id' => $trip->customer_id,
        ], $trip_details);
    ...
    public function my_current_trip(Request $request)
    {
       ...
       $query      = Trip::with('car', 'service', 'paymentMethod', 'driver', 'customer')->where($user_field, $user_id)->whereIn('status', $status)->orderBy('updated_at', 'desc');
    ...
    public function push_nearest_drivers($lat, $lng, $category_id )
    {
        ...
        //     $customer->customer->save();
        // }
        // $query = Driver::orderByDistance('location', $customer_location)->active()->available(); //find the nearest trip for the driver who is free(having no trip with status
        $drivers = Car::select('user_id')->where('category_id', '=', $category_id)
        // ->with('driver')
        ->whereHas('driver', function (Builder $query) use ($customer_location) {
            $query->orderByDistance('location', $customer_location);
            $query->active();
            $query->available();
        });
        // ->get();
        // return $drivers;
            ...
            // ->with('driver', function ($query) use ($customer_location) {
            //     $query->orderByDistance('location', $customer_location);
            //     $query->active();
            //     $query->available();
            // })

        //find the nearest trip for the driver who is free(having no trip with status 4,5,6,7,8)
