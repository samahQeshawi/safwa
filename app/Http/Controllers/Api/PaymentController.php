...
class PaymentController extends CustomersTripsController
{
	...
	//print_r($data);exit();
        if( $createRegistration ) {
            ...
            if(!empty($user->address) && !empty($user->city->name) && !empty($user->email)
            && !empty($user->zipcode) && !empty($user->name) && !empty($user->surname) ) {
                $data .= "&merchantTransactionId=". $request->id_no . "-" . md5(time()) .
                //   "&testMode=EXTERNAL" .
                ...
                // when comment this part checkout api will not show any validation errors
                "&billing.street1=" . $user->address .
                "&billing.city=" . $user->city->name .
                "&billing.state=" . $user->city->name .
                "&billing.country=SA" .
                "&billing.postcode=" . $user->zipcode .
                // =======================================================================
                "&customer.givenName=" . $user->name .
                "&customer.surname=" . $user->surname ;
            
            } else {
                $message = 'Part of Billing address is missing, make sure these information not empty (surname, email, address, city, zip code)';
                $status_code    = 500;
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            'Authorization:Bearer ' . env('ACCESS_TOKEN',"OGFjOWE0Yzg3YzAyMjNhODAxN2MxZjQzNTgwNzMyYWZ8SDJTNlBKTmIzaw==")));
    ...
    public function get_payment_token(Request $request)
    {
        ...
        if( $request->product_type == "trip" ) {
            ...
            if(!empty($query)) {
                ...
                $message = 'Faild to find this trip';
                $status_code    = 422;
                return response(['data' => null, 'message' => $message, 'status_code' => $status_code]);
            }
        } else if( $request->product_type == "booking" ) {
            $query = RentalCar::find($request->product_id);
            // $payment_method_id = $query->payment_method_id; // need to check
            if(!empty($query)) {
                $id_no = $query->booking_no;
                $payment_method_id = $query->payment_method_id;
            } else {
                $message = 'Faild to find this booking';
                $status_code    = 422;
                return response(['data' => null, 'message' => $message, 'status_code' => $status_code]);
            }
        ...
        if(isset($result['id'])) {
            ...
            $data['trackable_data'] = $result;

            if($request->product_type == 'trip') {
                Transaction::updateOrCreate(['trip_id' => $request->product_id], $data );
            }
            else if($request->product_type == 'booking') {
                Transaction::updateOrCreate(['booking_id' => $request->product_id], $data );
            }

            // Transaction::create($data);
            $message = 'successfully created checkout';
            $status_code    = 200;
        } else if(isset($result['status_code'])){
    ...
    public function get_payment_status(Request $request)
    {
        if($request->service_type == 'booking')
            $payment_method_id = RentalCar::find($request->id)->payment_method_id;
...
// 			test here
            if($status == 'successfully' || $status == 'succeeded') {
		...
		$tranaction_data = [];
	     	$tranaction_data['status'] = 'Failed';
                $tranaction_data['data'] = @$result['result'];
                $tranaction_data['trackable_data'] = @$result;
                $tranaction_data['amount'] = 0;
                $tranaction_data['brand'] = @$request->brand;
                $tranaction_data['note'] = 'Transaction Failed with card number: XXXXXXXXXXXX' . @$result['card']['last4Digits'];
                $tranaction_data['done_by'] = auth()->user()->id;
		 if($request->service_type == 'booking')
                    $tranaction_data['booking_id'] = @$request->id;
                else if($request->service_type == 'trip')
		...
		$tranaction_data['checkout_id'] = @$request->checkout_id;
		Transaction::create( $tranaction_data );
