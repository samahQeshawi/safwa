...
class AdminController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        */
        if ($request->ajax()) {
            ...
            $data = User::whereIn('admin_user',$ids)->with('permission')->get();
         ...
         $ids = Permission::pluck('id');
        $user_admin = User::whereIn('admin_user',$ids)->with('userType')->paginate(25);
    ...
    public function create()
    {
        ...
        return view('user_admin.create', compact('countries','cities','permissions'));
    }
    ...
    public function edit($id)
    {
        ...
        $cities = City::where('is_active',1)->pluck('name','id')->all();
        $permissions = Permission::all();
        return view('user_admin.edit', compact('user_admin','countries','cities','permissions'));
    }
    ...
    protected function getData(Request $request, $id=0)
    {
        $rules = [
            ...
            'email' =>  new EmailPhoneAdmin($id,'email', $request->email),
            'password' => 'nullable|string|min:1|max:255',
            'phone' =>  new EmailPhoneAdmin($id,'phone', $request->email),
            'admin_user' =>"nullable",
            'gender' => 'nullable',
            'country' => 'required',
            'city' => 'required',
        ...
        if ($id) {
            $rules = array_merge($rules,[
                'email' =>  [
                    ...
                    'min:1',
                    'max:255',
                     new EmailPhoneAdmin($id,'email', $request->email),
                ],
                'phone' =>  [
                    'required',
                    'min:1',
                    'max:20',
                     new EmailPhoneAdmin($id,'phone', $request->phone),
                ]
            ]);
        }
